...
const SignedOutMobileNav:FC = () => {
  ...
  return (
      <header className="md:hidden fixed bottom-0 z-10 flex justify-center  bg-white  w-full border-t border-gray-200  py-3 px-4  place-items-center transition-all ease-in duration-200">
  ...
  {navigationItems.map((item, index) => (
        ...
        >
          ...
          >
            {item.icon}
            <span className="text-[0.7rem] font-normal tracking-wide p-1">{item.label}</span>
          </button>
        </NavLink>
      ))}
  </div>
